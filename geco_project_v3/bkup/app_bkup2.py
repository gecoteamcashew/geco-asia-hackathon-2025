# app.py
# Cashew4Life - Dashboard (Patched Version 0.1)
# - Always use SKU_Description across pages
# - Robust SKU description mapping (SKU_Master + fallback from Sales_Transactions)
# - Campaign ROI: robust date/channel/spend handling
# - NLQ Query: year-only support, broader keywords, top-N sales queries, display "sales" instead of net_sales
# - Forecasts: Overall or per-SKU (by Description)
...
html, body, [data-testid="stAppViewContainer"] {
  font-size: 1.15rem;    /* bigger base font */
...
def normalize_ecommerce_purchases(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["discounts"] = (pd.Series(out["gross_sales"]) * pd.Series(out["discount_pct"]).astype(float)).astype(float)

    rr = out.get("returns_refunds", None)
    out["returns_refunds"] = coerce_float(rr, 0.0).astype(float) if rr is not None else 0.0

    out["net_sales"] = (out["gross_sales"] - out["discounts"] - out["returns_refunds"]).astype(float)
    out["campaign"] = out.get("campaign", "Unknown")

    keep = ["date","sku","quantity","unit_price","discount_pct","item_revenue","platform",
...
def normalize_sales_transactions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "item_revenue" in out.columns:
        ...
        out["item_revenue"] = (out["quantity"] * out["unit_price"] * (1 - out["discount_pct"]).astype(float)).astype(float)
...
# SKU master: cogs, inventory, and robust description map
def normalize_sku_master(df: pd.DataFrame) -> Dict[str, Union[pd.DataFrame, Dict[str, str]]]:
    ...
    if cost_like is None:
        ...
        if alt: cost_like = alt[0]
    cogs["cogs_per_unit"] = coerce_float(work[cost_like], 0.0).astype(float) if cost_like else 0.0
    ...
    if has_soh: inv["stock_on_hand"] = coerce_int(work["stock_on_hand"], 0).astype(int)
    if has_ltd: inv["lead_time_days"] = coerce_int(work["lead_time_days"], 14).astype(int)
    if has_soh or has_ltd:
        if not has_soh: inv["stock_on_hand"] = 0
        if not has_ltd: inv["lead_time_days"] = 14
...
def load_all() -> Dict[str, Union[pd.DataFrame, Dict]]:
    ...
    purchases = pd.concat([df for df in [norm_ep, norm_st] if not df.empty], ignore_index=True)
    ...
    if not traffic.empty:
        ...
        traffic["date"] = coerce_date(traffic.get("date", pd.Series(pd.NaT)), fallback=pd.Timestamp("2023-01-01"))
...
def diagnostics(d: Dict[str, pd.DataFrame]):
    with st.expander("Diagnostics"):
        st.write("Detected files:", d["files"])
        st.write("Raw Ecommerce_Purchases columns:", list(d["raw_ep"].columns))
        st.write("Raw Sales_Transactions columns:", list(d["raw_st"].columns))
        st.write("Normalized purchases columns:", list(d["purchases"].columns))
        st.write("Row counts:", {
            "raw_ep": len(d["raw_ep"]),
            "raw_st": len(d["raw_st"]),
            "normalized": len(d["purchases"]),
        })
        p = d["purchases"]
        if not p.empty:
            st.write(p.head(10))
            st.write({
                "quantity_sum": float(p["quantity"].sum()),
                "unit_price_sum": float(p["unit_price"].sum()),
                "item_revenue_sum": float(p["item_revenue"].sum()),
                "net_sales_sum": float(p["net_sales"].sum()),
                "contribution_margin_sum": float(p.get("contribution_margin", pd.Series(dtype=float)).sum()),
            })
...
def page_home(d: Dict[str, pd.DataFrame]):
    ...
    if purchases.empty or (
        float(purchases["quantity"].sum()) == 0.0 and float(purchases["net_sales"].sum()) == 0.0 and float(purchases["item_revenue"].sum()) == 0.0
    ):
        st.error("Purchases still zero. See Diagnostics for raw headers. If needed, share head -n 2 for both CSVs.")
        return

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Total Net Sales", f"${purchases['net_sales'].sum():,.0f}")
    c2.metric("Total Units Sold", f"{int(purchases['quantity'].sum()):,}")
    c3.metric("Contribution Margin", f"${purchases.get('contribution_margin', pd.Series(dtype=float)).sum():,.0f}")
    c4.metric("Rows", f"{len(purchases):,}")

    st.markdown("---")
    st.subheader("Top SKUs by Sales")
    by_sku = (purchases.groupby("SKU_Description", dropna=False)["net_sales"]
              .sum().reset_index().sort_values("net_sales", ascending=False))
    # rename for display so the y-axis reads "sales"
...
def page_platform(d: Dict[str, pd.DataFrame]):
    ...
    if p.empty:
        ...
        return
    plat = p.groupby("platform", dropna=False).agg(
    ...
    if not plat.empty:
        fig = px.bar(plat, x="platform", y=["gross_sales","discounts","returns","net_sales"], barmode="group")
        st.plotly_chart(fig, use_container_width=True)


def page_products(d: Dict[str, pd.DataFrame]):
    p = d["purchases"]; inv = d["inventory"]
    if p.empty:
        st.warning("No purchases data.")
        return

    prod = p.groupby("SKU_Description", dropna=False).agg(
        total_net_sales=("net_sales", "sum"),
        total_units=("quantity", "sum"),
        contribution_margin=("contribution_margin", "sum"),
    ).reset_index().sort_values("total_net_sales", ascending=False)
    # Display with sales naming
    prod_disp = prod.rename(columns={"total_net_sales": "total_sales"})
    st.dataframe(prod_disp.rename(columns={"SKU_Description":"SKU"}), use_container_width=True)
    if not prod.empty:
        fig = px.bar(prod.sort_values("contribution_margin", ascending=False).head(12),
                     x="SKU_Description", y="contribution_margin")
        fig.update_layout(xaxis_title="SKU (Description)")
        st.plotly_chart(fig, use_container_width=True)
    ...
    if inv.empty:
        st.info("No inventory cols (stock_on_hand/lead_time_days) in SKU_Master.csv.")
    else:
...
def page_forecasts(d: Dict[str, pd.DataFrame]):
    ...
    try:
        model = ExponentialSmoothing(s, seasonal_periods=12, trend="add", seasonal="add", initialization_method="estimated")
        fit = model.fit()
        fc = fit.forecast(6)
    ...
    if not fc_df.empty:
        fig.add_scatter(x=fc_df["month"], y=fc_df["forecast_quantity"], name="Forecast", mode="lines")
    st.plotly_chart(fig, use_container_width=True)
...
def page_campaign_roi(d: Dict[str, pd.DataFrame]):
    ...
    by_rev = (rev.groupby(["date_only", "channel"], as_index=False)
                .agg(revenue_attributed=("net_sales","sum")))
    by_rev["date"] = pd.to_datetime(by_rev["date_only"])
    by_rev = by_rev.drop(columns=["date_only"])

    if not t.empty:
        spend = t.copy()
        spend["date"] = coerce_date(spend.get("date", pd.Series(pd.NaT)), fallback=pd.Timestamp("2023-01-01"))
        ...
        agg_spend = (spend.groupby(["date_only", "channel"], as_index=False)
                        .agg(spend=("spend","sum")))
    ...
    roi = agg_spend.merge(by_rev, on=["date","channel"], how="left").fillna({"revenue_attributed": 0.0})
    if roi["spend"].sum() == 0:
        st.info("No marketing spend found; ROAS uses 0 spend fallback.")
    roi["ROAS"] = np.where(roi["spend"] > 0, roi["revenue_attributed"] / roi["spend"], np.nan)
    ...
    c3.metric("Avg ROAS", f"{(f['revenue_attributed'].sum() / f['spend'].sum()):.2f}" if f['spend'].sum() else "N/A")

    if not f.empty:
        tline = f.groupby("date", as_index=False)[["spend","revenue_attributed"]].sum()
        fig1 = px.line(tline, x="date", y=["spend","revenue_attributed"], title="Spend vs Attributed Revenue (Daily)")
        st.plotly_chart(fig1, use_container_width=True)

        tbar = f.groupby("channel", as_index=False)[["spend","revenue_attributed"]].sum()
        fig2 = px.bar(tbar, x="channel", y=["spend","revenue_attributed"], barmode="group", title="By Channel")
...
def page_nlq(d: Dict[str, pd.DataFrame]):
    ...
    st.caption("Try: 'revenue shopee pistachios oct 2024', 'orders lazada honey almonds jul 2024', 'qty shopify pistachios 2023', 'top 5 sales 2024', 'top 10 sales shopee pistachios 2023'")
    q = st.text_input("Query", "")

    def run_query(q: str, df: pd.DataFrame) -> pd.DataFrame:
        ql = q.lower().strip()
        res = df.copy()

        # Platform / channel keywords (exact tokens + generic groupings)
        for ch in ["lazada","shopee","shopify","direct","email","referral","organic search","paid search","social"]:
            if ch in ql:
                token = ch.split()[0]
        ...
        prod_keywords = [
            "honey almond","honey almonds",
            "roasted cashew","roasted cashews",
            "natural cocktail mix",
            "fancy mixed nuts",
            "natural baked cashew","natural baked cashews",
            "natural baked almond","natural baked almonds",
            "walnut butter",
            "pistachio","pistachios",
            "cranberry","cranberries",
            "dried mango",
            "smoked almond","smoked almonds",
            "satay cashew","satay cashews",
            "peanut","peanuts"
        ]
        ...
        # Date parsing: month-year or year-only
        months = {"jan":1,"feb":2,"mar":3,"apr":4,"may":5,"jun":6,"jul":7,"aug":8,"sep":9,"oct":10,"nov":11,"dec":12}
        m = re.search(r"(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\s+(\d{4})", ql)
        if m:
            mo = months[m.group(1)]
            yr = int(m.group(2))
        ...
        # Metrics tiles based on keywords
        ...
        # Top N queries (e.g., "top 5 sales", optionally with platform/product filters in the same query)
        topn_match = re.search(r"top\s+(\d+)\s+sales", ql)
        if topn_match:
            n = int(topn_match.group(1))
            ...
            fig = px.bar(agg.head(n).rename(columns={"net_sales":"sales"}), x="SKU_Description", y="sales", title=f"Top {n} Sales")
            st.plotly_chart(fig, use_container_width=True)

        # Output table renamed for readability
        ...
        out = res[cols_show].sort_values("date").rename(columns={"net_sales":"sales","customer_id":"Customer"})
        return out
