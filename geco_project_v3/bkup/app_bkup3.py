# app.py
#   Cashew4Life - Dashboard (Latest Patched Version 0.1b)
...
def page_products(d: Dict[str, pd.DataFrame]):
    ...
    if not inv.empty:   # 👈 only show if we have inventory
...
def page_nlq(d: Dict[str, pd.DataFrame]):
    ...
    st.caption("Try: 'revenue shopee pistachios oct 2024', 'orders lazada honey almonds jul 2024', 'qty shopify pistachios 2023', 'top five product sales'")
    ...
    def run_query(q: str, df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        # Product filter
        prod_keywords = ["honey almond","roasted cashew","natural cocktail mix","pistachio","cranberry","dried mango","smoked almond","satay cashew","peanut"]
        ...
        m = re.search(r"(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|january|february|march|april|june|july|august|september|october|november|december)\s+(\d{4})", ql)
        if m:
            matched_any = True
            mo = months[m.group(1)]
            yr = int(m.group(2))
        ...
        # Top N queries
        topn_match = re.search(r"top\s+(\d+|five)\s+.*sales", ql)
        if topn_match:
            matched_any = True
            n_raw = topn_match.group(1)
            n = 5 if n_raw == "five" else int(n_raw)
    ...
    if q:
        ...
        if out is not None and isinstance(out, pd.DataFrame) and not out.empty:
            ...
            st.dataframe(out[cols_show].sort_values("date").rename(columns={"net_sales":"sales","customer_id":"Customer"}), use_container_width=True)
    else:
